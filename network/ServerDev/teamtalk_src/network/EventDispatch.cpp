...
void CEventDispatch::StartDispatch(uint32_t wait_timeout)
{
	...
	timeval timeout;
	timeout.tv_sec = 0;
	timeout.tv_usec = wait_timeout * 1000;	// 100 millisecond
    ...
    while (running)
	{
		...
		{
			LOG__("select failed, error code: %d", GetLastError());
			Sleep(timeout.tv_usec);
		...
		{
			//LOG__("select return read count=%d\n", read_set.fd_count);
		...
		{
			//LOG__("select return write count=%d\n", write_set.fd_count);
		...
		{
			//LOG__("select return exception count=%d\n", excep_set.fd_count);
...
void CEventDispatch::AddEvent(SOCKET fd, uint8_t socket_event)
{
	// socket_event参数没有用到，AddEvent把所有事件都添加进去了，因为EPOLLET模式不会造成频繁触发事件
	struct epoll_event ev;
	ev.events = EPOLLIN | EPOLLOUT | EPOLLET | EPOLLPRI | EPOLLERR | EPOLLHUP;
	ev.data.fd = fd;
	if (epoll_ctl(m_epfd, EPOLL_CTL_ADD, fd, &ev) != 0)
	{
		LOG__("epoll_ctl() failed, errno=%d", errno);
	}
}
...
void CEventDispatch::StartDispatch(uint32_t wait_timeout)
{
	...
	while (running)
	{
		...
		for (int i = 0; i < nfds; i++)
		{
            ...
            {
                //LOG__("On Peer Close, socket=%d, ev_fd);
                pSocket->OnClose();
            }
            #endif
            // Commit End
			...
			{
				//LOG__("OnRead, socket=%d\n", ev_fd);
				pSocket->OnRead();
			}

			if (events[i].events & EPOLLOUT)
			{
				//LOG__("OnWrite, socket=%d\n", ev_fd);
				pSocket->OnWrite();
			}

			if (events[i].events & (EPOLLPRI | EPOLLERR | EPOLLHUP))
			{
				//LOG__("OnClose, socket=%d\n", ev_fd);
				pSocket->OnClose();
			}
